...
次判定: 工程1、工程2の品質Gate合格後、本書を工程3成果として再検証する

## 0. この文書の役割

この文書は、最初に商品品質まで作る1F〜10Fについて、各階の目的、敵、猫、施設、解放条件、初回導線、戦闘の読み方、制圧後の生活、失敗時の改善案、音と動き、保存境界を実装可能な粒度で固定する。

本書の製品内容は削除・改変せず、旧工程3の成果候補として保持する。ただし、現行の品質Gateによる再検証前のため完成成果ではない。工程1、工程2、本書が順に`PASS`するまで、工程4以降の完成判定や新規着手の根拠に使わない。
...
工程1、工程2、本書が品質Gateを順に`PASS`した後の工程4で、次の9画面を完成見本として作る。
...
以下のチェックは旧判定時の達成記録であり、現在の`PASS`を意味しない。工程1合格後、現行の完成判定Gateで実物・反証・判定を再実行する。
...
旧判定では、これらを工程3の欠落ではなく、順番どおり後工程で確定する項目としていた。再検証ではこの分離自体が妥当かも確認する。工程1、工程2、本書の合格後、工程4で本書の9画面以外を増やす前に、9画面で全中核導線を表現できるかを検証する。
