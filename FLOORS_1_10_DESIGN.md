...
工程4で、上位正本`MASTER_SPEC.md` §14の機能契約に従い次の9画面を完成見本として作成した。正本は`NINE_SCREEN_MOCKUPS.md`とする。
...
1F初回戦闘は`S02`のチュートリアル状態、10Fボス第2形態は`S02`のボス状態、敗北診断は`S02`から開く補助シートとして静止差分まで作成した。独立した主要画面へ数えて上位正本の9画面を押し出さない。工程5では、この静止差分へ時間、入力キャンセル、音と視覚の同期を追加する。
...
| 9画面の具体レイアウト | `PASS` | 工程4・`NINE_SCREEN_MOCKUPS.md` |
| 全動作のフレーム割り | 未制作 | 工程5 |
| 色、線、材質、形状文法 | 未制作 | 工程6 |
| 実素材の可読性 | 未検証 | 工程7 |
...
残る項目は工程3の欠落ではなく、順番どおり後工程で確定する。工程4では9画面ファミリーと4つの必須状態差分で全中核導線の受け皿を確認した。
