...
工程4で、上位正本`MASTER_SPEC.md` §14の機能契約に従い次の9画面を構造ワイヤーフレームとして作成した。構造正本は`NINE_SCREEN_MOCKUPS.md`とする。本番美術完成見本は制作後の品質レビューで不合格となり、工程4は`IN_PROGRESS`へ戻している。
...
1F初回戦闘は`S02`のチュートリアル状態、10Fボス第2形態は`S02`のボス状態、敗北診断は`S02`から開く補助シートとして静止差分まで作成した。独立した主要画面へ数えて上位正本の9画面を押し出さない。工程4の完成品質へ合格するまでは工程5へ進まない。
...
| 9画面の構造レイアウト | `PASS` | 工程4・`NINE_SCREEN_MOCKUPS.md` |
| 9画面の本番美術完成品質 | `IN_PROGRESS` | 工程4・`QUALITY_GATE.md` |
| 全動作のフレーム割り | 未制作 | 工程5 |
| 色、線、材質、形状文法 | 未制作 | 工程6 |
| 実素材の可読性 | 未検証 | 工程7 |
...
残る項目は工程3の欠落ではない。工程4では9画面ファミリーと4つの必須状態差分で全中核導線の受け皿を確認したが、アート方針と試験素材を後回しにしたため、完成見本の品質には届かなかった。再制作では工程順を組み替え、重要3画面の実物品質を先に合格させる。
