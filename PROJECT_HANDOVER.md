...
工程4「9画面の完成見本」は[`NINE_SCREEN_MOCKUPS.md`](./NINE_SCREEN_MOCKUPS.md)で`PASS`となった。390×844基準のヒーロー9枚に加え、1Fチュートリアル、敗北診断、10Fボス第2段階、100F全塔制覇の必須状態差分4枚を固定した。ゲーム本体コードは変更していない。
...
現在の次作業は工程5「動きの絵コンテ」である。呼び鈴の100ms反応と400ms長押し、入口出現、実移動、接触、命中、撃破、階段上昇、物理配送、ボス予兆、敗北から直接修正、夜明け実行を時間軸へ落とす。画面階層を再設計したり、ゲーム本体コードへ着手したりしない。

工程4で後工程へ送ったのは、最終HP、価格、DPS、倍率、最終素材、正式アンカーJSON、物理端末QAである。構造、役割、因果、表示要件、床座標式は確定済み。
