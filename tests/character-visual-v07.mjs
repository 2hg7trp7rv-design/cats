...
const out = process.env.CATS_TEST_OUT || `test-results/v07-${browserName}`;
...
const state = {version:'0.7.0',coins:1086,parts:0,floors,cats,bellAt:now+600000,settings:{sound:false},tutorial:true,coach:{battle:true},sales:7,built:6,clears:1,lastBattle:now,lastSeen:now,aidAt:now,aidTotal:0,created:now,battle:null};
...
await page.screenshot({path:`${out}/01-tower.png`,fullPage:true});

await page.locator('[data-nav="cats"]').tap();
await page.locator('.sheet').waitFor({state:'visible',timeout:5000});
const listImgs=page.locator('.sheet .catCard .catSprite');
...
await page.locator('.sheet header [data-close="1"]').tap();
...
for(const id of ['mugi','luna','toto','mimi']){
  ...
  await page.screenshot({path:`${out}/profile-${id}.png`,fullPage:true});
  await page.locator('.sheet header [data-close="1"]').tap();
}
...
await writeFile(`${out}/report.json`,JSON.stringify({passed:true,browserName,targetUrl,assetResults,towerHeights,listMetrics,profileHeights,errors},null,2));
await browser.close();
console.log(`V0.7 character visual QA passed: ${browserName} ${targetUrl}`);
