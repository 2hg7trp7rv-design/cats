...
工程4の画面構造、共通座標案、必須状態差分、レスポンシブ規則は[`NINE_SCREEN_MOCKUPS.md`](./NINE_SCREEN_MOCKUPS.md)を構造正本とする。ヒーロー9枚は[`NINE_SCREEN_MOCKUPS.html`](./NINE_SCREEN_MOCKUPS.html)、1Fチュートリアル、敗北診断、10Fボス、100F全塔制覇は[`NINE_SCREEN_VARIANTS.html`](./NINE_SCREEN_VARIANTS.html)で確認する。これらは本番美術完成見本ではなく、完成判定には[`QUALITY_GATE.md`](./QUALITY_GATE.md)のG1〜G5を別途必要とする。
...
制作完了後は[`QUALITY_GATE.md`](./QUALITY_GATE.md)に従い、要求適合、最終成果物の実物品質、人間期待、比較・反証、通常利用を制作者とは別の批評者・判定者視点で再確認する。一つでも不合格なら`IN_PROGRESS`へ戻し、完成報告せず要求定義または構成からやり直す。
...
4. 9画面の完成見本
   - 構造ワイヤーだけでは完了しない。
   - ミニアート方針と少数試験素材を先行し、戦闘、塔閲覧、店舗変身の重要3画面を本番品質で合格させる。
   - 重要3画面の合格後に残り6画面へ展開する。
5. 動きの絵コンテ（工程4全体の合格後）
6. アートバイブル（工程4用に先行した方針と素材を正式化）
7. 少数の試験素材（工程4用の先行試作を正式検証）
8. 保存・100Fデータ設計
9. QA合格表
10. コード修正
